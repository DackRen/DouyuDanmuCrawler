...
public class DyMsgFactory {
	...
	public static byte[] parseRecvMsg(byte[] recvBytes,int recvLen, Collection<MsgMapper> mappers){
		...
		if(leftBytes == null){
			...
			try {
				...
				int index = Formatting.indexOf(leftBytes, "type@=".getBytes(),12);
				if(index != -1){
    ...
    private static int parseMsgHead(byte[] head) throws CheckException{
    	...
    	int length1 = Formatting.fromLH(head1) ;
    	int length2 = Formatting.fromLH(head2) ;
    	if((length1 != length2) || length1 < 12){
    		throw new CheckException("Bad Head!");
    	}

    	if(Formatting.fromLH(messageType) != 690){
    		//return Formating.fromLH(head1) +4;
    		throw new CheckException("Bad Head!" + String.valueOf(Formatting.fromLH(messageType)));
    	}else{
    		return Formatting.fromLH(head1) +4;
    	}
    }
    ...
    private static byte[] getByte(String data){
         ...
         {
             boutput.reset();
             doutput.write(Formatting.toLH(data.length() + 8), 0, 4);        // 4 bytes packet length
             doutput.write(Formatting.toLH(data.length() + 8), 0, 4);        // 4 bytes packet length
             doutput.write(Formatting.toLH(DY_MESSAGE_TYPE_CLIENT), 0, 2);   // 2 bytes message type
